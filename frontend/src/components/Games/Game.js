...
function Game() {
  ...
  const DrawBoard = () => {
    ...
    if (form.id !== 0 && myTurn && !finPartida) {
      movimientos.map(movimiento => {
        ...
        if (color === 'BLACK') {
          ctx.fillRect(
            700 - movimiento[0] * 100,
            700 - movimiento[1] * 100,
            100,
            100,
          )
        } else {
    ...
    pieces.map(piece => {
      const pieza = document.getElementById(piece.type + '-' + piece.color)
      ...
      if (
        ...
        if (color === 'BLACK') {
          ctx.fillRect(
            700 - piece.xposition * 100,
            700 - piece.yposition * 100,
            100,
            100,
          )
        } else {
  ...
  const oMousePos = evt => {
    ...
    const x = Math.floor(Math.round(evt.clientX - ClientRect.left) / 100)
    const y = Math.floor(Math.round(evt.clientY - ClientRect.top) / 100)
      ...
      pieces.map(piece => {
        if (color === 'BLACK' && myTurn && !coronacion) {
          if (
            ...
            const url = apiUrl + '/games/listMovements/' + piece.id
        ...
        movimientos.map(movimiento => {
          if (color === 'BLACK') {
            if (7 - movimiento[0] === x && 7 - movimiento[1] === y) {
              setForm({
                ...form,
                xposition: movimiento[0],
                yposition: movimiento[1],
              })
            }
          } else {
  ...
  async function InicioTurno() {
    ...
    const url = apiUrl + sampleLocation.pathname + '/startTurn'
  ...
  const finTiempo = () => {
    ...
    const url = apiUrl + sampleLocation.pathname + '/endTime'
    axios
      .get(url, { headers: { Authorization: `Bearer  ${token}` } })
      .then(response => {
  ...
  const partida = () => {
    ...
    axios
      ...
      .then(response => {
        ...
        if (
          response.data[0].coronacion === true &&
          response.data[0].turn === response.data[1]
        ) {
          setForm({
            id: response.data[0].idCoronacion,
            xposition: '-1',
            yposition: '-1',
            type: 'QUEEN',
          })
        ...
        if (
          response.data[0].jaqueMate === true &&
          response.data[0].jaque === true &&
          response.data[0].turn === response.data[1]
        ) {
          document.getElementById('msg').innerHTML =
          ...
          setTimeOpponent(0)
          document.getElementById('msg').innerHTML =
            '¡¡Has ganado la partida por tiempo!!'
        } else if (
          response.data[2] === true &&
          response.data[0].turn !== response.data[1] &&
          response.data[1] !== 'espectador'
  ...
  const mover = () => {
    ...
    const url = apiUrl + sampleLocation.pathname + '/move'

    axios
      .post(url, form, { headers: { Authorization: `Bearer  ${token}` } })
      .then(response => {
        setTurn(response.data[0].turn)
        setJaque(response.data[0].jaque)
        setFinPartida(response.data[1])

        if (response.data[0].coronacion === true) {
          // Si la coronacion es true, no cambio el turno hasta que seleccione una pieza para sustituir
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (
        (myTurn || (color === 'espectador' && turn === 'WHITE')) &&
        !finPartida
      ) {
        ...
        ((!myTurn && inicializado === 'true' && color !== 'espectador') ||
          (color === 'espectador' && turn === 'BLACK')) &&
        !finPartida
      ) {
        refresco()
      }
  ...
  const handleButton = () => {
    ...
    const url = apiUrl + sampleLocation.pathname + '/move/coronacion'
  ...
  return (
    <React.Fragment>
      ...
      <div className='container'>
        ...
        <canvas id='canvas' width={800} height={800} onClick={oMousePos}>
          {' '}
        </canvas>
        <img
          id='source'
          src={require('../../assets/img/tablero.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        ...
        <img
          id='HORSE-BLACK'
          src={require('../../assets/img/HORSE-BLACK.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='HORSE-WHITE'
          src={require('../../assets/img/HORSE-WHITE.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='KING-BLACK'
          src={require('../../assets/img/KING-BLACK.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='KING-WHITE'
          src={require('../../assets/img/KING-WHITE.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='BISHOP-BLACK'
          src={require('../../assets/img/BISHOP-BLACK.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='BISHOP-WHITE'
          src={require('../../assets/img/BISHOP-WHITE.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='PAWN-BLACK'
          src={require('../../assets/img/PAWN-BLACK.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='PAWN-WHITE'
          src={require('../../assets/img/PAWN-WHITE.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='TOWER-WHITE'
          src={require('../../assets/img/TOWER-WHITE.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='TOWER-BLACK'
          src={require('../../assets/img/TOWER-BLACK.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='QUEEN-WHITE'
          src={require('../../assets/img/QUEEN-WHITE.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        <img
          id='QUEEN-BLACK'
          src={require('../../assets/img/QUEEN-BLACK.png')}
          alt='alt'
          style={{ display: 'none' }}
        />
        ...
        {inicializado === 'true' && (
          <div>
            ...
            {myTurn && !finPartida && (
              <div>
                <h1 className='timeTurn'>
                  {' '}
                  {Math.floor(time / 60)}:
                  {time % 60 < 10 ? '0' + (time % 60) : time % 60}
                </h1>

                <h1 className='timeOpponent'>
                  {' '}
                  {Math.floor(timeOpponent / 60)}:
                  {timeOpponent % 60 < 10
                    ? '0' + (timeOpponent % 60)
                    : timeOpponent % 60}
                </h1>
            ...
            {!myTurn && color !== 'espectador' && !finPartida && (
              <div>
                <h1 className='timeTurnOpponent'>
                  {' '}
                  {Math.floor(time / 60)}:
                  {time % 60 < 10 ? '0' + (time % 60) : time % 60}
                </h1>

                <h1 className='timeOpponentTurn'>
                  {' '}
                  {Math.floor(timeOpponent / 60)}:
                  {timeOpponent % 60 < 10
                    ? '0' + (timeOpponent % 60)
