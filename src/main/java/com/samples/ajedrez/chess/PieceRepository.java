...
public interface PieceRepository extends CrudRepository<Piece,Integer> {
    ...
    @Query("SELECT piece FROM Piece piece WHERE piece.xPosition = :x AND piece.yPosition = :y AND piece.board.id = :boardId AND piece.color != :color")
	Optional<Piece> comprobarCasilla(int x, int y,int boardId,String color) throws DataAccessException;
    ...
    @Query("SELECT piece FROM Piece piece WHERE piece.peonPaso = true AND piece.board.id = :boardId AND piece.type = 'PAWN'")
    Optional<Piece> peonPaso(int boardId) throws DataAccessException;
